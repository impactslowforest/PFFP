...
const FIELD_LABELS = {
    ...
    Supported: { 'Support_ID': 'thSupportID', 'Support_Type': 'thSupportCode', 'Date': 'thDate', 'Species_Name': 'thItemDetail', 'Quantity': 'thQuantity', 'Unit': 'thUnit', 'Note': 'thNotes', 'Staff_Input': 'thStaff', 'A_live': 'thAlive', 'Program': 'thSupportedBy', 'Year': 'thSupportedYear' },
...
const FIELD_MAPPING = {
    ...
    'Supported': {
        'Support_Type': { map: 'drop', condition: 'Support list' },
        'Species_Name': 'species',
        'Unit': { map: 'drop', condition: 'Unit' },
        'Program': { map: 'drop', condition: 'Organization', separator: ',' },
        'Year': { map: 'drop', condition: 'Participation Year' },
        'Staff_Input': 'user'
    },
    'Op6': {
...
function loadData(forceRefresh = false) {
    ...
    var optionalQueries = [
        ...
        supabaseClient.from('farmer_year').select('*').limit(10000).then(function (r) { return r; }).catch(function () { return { data: [], error: null }; }),
        supabaseClient.from('survival_check').select('*').limit(10000).then(function (r) { return r; }).catch(function () { return { data: [], error: null }; })
    ];
    Promise.all(coreQueries.concat(optionalQueries)).then(function (results) {
        ...
        var farmerYearData = (results[10] && !results[10].error) ? (results[10].data || []) : [];
        var survivalCheckData = (results[11] && !results[11].error) ? (results[11].data || []) : [];
        console.log('OP6 data loaded:', op6Data.length, 'records', op6Data.length > 0 ? 'keys=' + Object.keys(op6Data[0]).join(',') : '(empty)');
        console.log('Farmer_Year loaded:', farmerYearData.length, 'records');
        console.log('Survival_Check loaded:', survivalCheckData.length, 'records');
        var data = {
            ...
            op6: op6Data,
            farmer_year: farmerYearData,
            survival_check: survivalCheckData
        };
...
function onDataLoaded(data, fromCache = false) {
    ...
    try {
        ...
        filteredData = { farmers: rawData.farmers || [], plots: rawData.plots || [], yearly: rawData.yearly || [], supported: rawData.supported || [], farmer_year: rawData.farmer_year || [], survival_check: rawData.survival_check || [] };
        initFilters();
...
function resolveValue(key, value, tName) {
    ...
    // For Supported Species_Name (was Item_Detail): try speciesMap first, then trainingListMap
    if (tName === 'Supported' && key === 'Species_Name' && mK === 'species') {
...
function initFilters() {
    ...
    // 8. Supported Types - from Supported table (Support_Type field)
    let supTS = new Set(); (rawData.supported || []).forEach(s => { var code = s['Support_Type'] || ''; if (code) supTS.add(code.trim()); });
    ...
    requestAnimationFrame(() => {
        ...
        if (fYSup !== 'All' || fST !== 'All') {
            let matchedSupp = (rawData.supported || []).filter(s => {
                if (fYSup !== 'All') {
                    var sy = String(s['Year'] || '');
                    if (!fYSup.includes(sy)) return false;
                }
                if (fST !== 'All') {
                    var sc = String(s['Support_Type'] || '');
                    if (!fST.includes(sc)) return false;
                }
                return true;
            });
            suppFilterFIDs = new Set(matchedSupp.map(s => s.Farmer_ID));
        }
        ...
        let filtS = (rawData.supported || []).filter(s => {
            if (!fIDs.has(s.Farmer_ID)) return false;
            if (fYSup !== 'All') {
                var sy = String(s['Year'] || '');
                if (!fYSup.includes(sy)) return false;
            }
        ...
        // Filter survival_check by Plot_IDs from filtered plots
        let plotIDs = new Set(filtP.map(p => p['Plot_Id']));
        let filtSC = (rawData.survival_check || []).filter(sc => plotIDs.has(sc['Plot_ID']));
        // Filter farmer_year by filtered farmer IDs
        let filtFY = (rawData.farmer_year || []).filter(fy => fIDs.has(fy['Farmer_ID']));
        filteredData = { farmers: filtF, plots: filtP, yearly: filtY, supported: filtS, survival_check: filtSC, farmer_year: filtFY };
        updateUI(filtF, filtP, filtY, filtS, filtSC);
    });
}

function updateUI(f, p, y, s, sc) {
    ...
    // KPI3: Total Coffee Area from Farmers
    ...
    treeRecs.forEach(x => { if (x.Species_Name) { if (selS === 'All' || selS.includes(x.Species_Name)) suppSpecies.add(x.Species_Name); } });
    ...
    window._survivalCheckData = scData;
...
function drawAnalyticsCharts(f, p, y) {
    ...
    var suppAlive = suppEvalRecs.reduce((a, b) => a + (parseFloat(b.A_live) || 0), 0);
...
function drawSurvivalRateChart(farmers, yearly) {
    ...
    var treeRecs = supported.filter(s => fIDs.has(s.Farmer_ID) && (s.Unit || '').toLowerCase().indexOf('tree') >= 0);
    var evaluated = treeRecs.filter(s => { var a = (s.A_live); return a !== null && a !== '' && a !== undefined && String(a).trim() !== ''; });
    ...
    evaluated.forEach(s => {
        ...
        if (info) {
            ...
            villageData[info.label].alive += parseFloat(s.A_live) || 0;
...
function showFarmerDetails(farmerId) {
    ...
    let relatedSupported = (rawData.supported || []).filter(s => String(s.Farmer_ID) === String(farmerId));
    relatedSupported.sort((a, b) => (b.Year || '').localeCompare(a.Year || ''));
...
function showAddSupportedModal(farmerId) {
    ...
    html += '<input type="text" class="form-control form-control-sm bg-light" name="Farmer_ID" value="' + escapeHtml(farmerId) + '" readonly>';
    html += '</div>';
    ...
    html += '<select class="form-select form-select-sm" name="Year" id="supportYear">';
    ...
    html += '<select class="form-select form-select-sm" name="Support_Type" id="supportCodeSelect" onchange="onSupportCodeChange(this.value)">';
    ...
    html += '<select class="form-select form-select-sm select2-generic" name="Program" multiple>';
    ...
    html += '<select class="form-select form-select-sm select2-generic" name="Species_Name" multiple>';
    Object.keys(speciesMap).forEach(function (id) {
    ...
    html += '<input type="text" class="form-control form-control-sm" name="Species_Name" list="dl_fertType" placeholder="' + (currentLang === 'vi' ? 'Nhập hoặc chọn...' : 'Type or select...') + '">';
    ...
    html += '<input type="date" class="form-control form-control-sm" name="Species_Name" value="' + todayStr + '">';
    html += '</div>';
    html += '</div></div>';
    ...
    html += '<select class="form-select form-select-sm" name="Species_Name">';
...
function openEditSupportedForm(item) {
    ...
    var html = '';
    var supportCode = item.Support_Type || '';
    ...
    html += '<div class="col-md-6 col-lg-4 mb-3">';
    html += '<label class="form-label-custom">' + (t.thSupportedYear || 'Supported Year') + '</label>';
    html += '<select class="form-select form-select-sm" name="Year">';
    html += '<option value="">-- ' + (currentLang === 'vi' ? 'Chọn' : 'Select') + ' --</option>';
    var yearKeys = Object.keys(dropMap).filter(function (id) { return dropMap[id].condition === 'Participation Year'; });
    yearKeys.forEach(function (id) {
        var lbl = (currentLang === 'vi' ? dropMap[id].vi : dropMap[id].en) || id;
        var sel = (id === String(item.Year || '')) ? ' selected' : '';
        html += '<option value="' + id + '"' + sel + '>' + lbl + '</option>';
    });
    html += '</select></div>';
    ...
    html += '<div class="col-md-6 col-lg-4 mb-3">';
    html += '<label class="form-label-custom">' + (t.thSupportedBy || 'Supported By') + '</label>';
    var existOrgs = String(item.Program || '').split(',').map(function (s) { return s.trim(); });
    html += '<select class="form-select form-select-sm select2-generic" name="Program" multiple>';
    var orgKeys = Object.keys(dropMap).filter(function (id) { return dropMap[id].condition === 'Organization'; });
    orgKeys.forEach(function (id) {
        var lbl = (currentLang === 'vi' ? dropMap[id].vi : dropMap[id].en) || id;
    ...
    html += '<input type="number" class="form-control form-control-sm" name="A_live" value="' + escapeHtml(item.A_live || '') + '" step="1" min="0">';
    html += '</div>';
    ...
    var existSpecies = String(item.Species_Name || '').split(',').map(function (s) { return s.trim(); });
    ...
    html += '<div class="col-md-6 col-lg-4 mb-3">';
    html += '<label class="form-label-custom">' + (t.thFertType || 'Loại phân bón') + '</label>';
    html += '<input type="text" class="form-control form-control-sm" name="Species_Name" list="dl_fertType" value="' + escapeHtml(activeGroup === 'fertilizer' ? (item.Species_Name || '') : '') + '">';
    html += '<datalist id="dl_fertType">';
    Object.keys(dropMap).forEach(function (id) {
        if (dropMap[id].condition === 'Fertilizer') {
            var lbl = (currentLang === 'vi' ? dropMap[id].vi : dropMap[id].en) || id;
    ...
    html += '<div class="col-md-6 col-lg-4 mb-3">';
    html += '<label class="form-label-custom">' + (t.thSampleDate || 'Ngày xét nghiệm') + '</label>';
    html += '<input type="date" class="form-control form-control-sm" name="Species_Name" value="' + escapeHtml(activeGroup === 'soiltest' ? (item.Species_Name || '') : '') + '">';
    html += '</div>';
    html += '</div></div>';
    ...
    Object.keys(trainingListMap).forEach(function (id) {
        var lbl = (currentLang === 'vi' ? trainingListMap[id].vi : trainingListMap[id].en) || id;
        var sel = (activeGroup === 'training' && id === String(item.Species_Name || '')) ? ' selected' : '';
        html += '<option value="' + id + '"' + sel + '>' + lbl + '</option>';
    });
    html += '</select></div>';
    html += '<div class="col-md-6 col-lg-4 mb-3">';
...
async function deleteItem(type, id) {
    ...
    if (type === 'Plots') {
        ...
        if (delSup) { recalcFarmerId = delSup.Farmer_ID; recalcYear = delSup.Year; }
    }
...
function saveData() {
    ...
    if (formType === 'Farmers') {
        ...
        if (trainingTime && formData.Support_Type === 'SP7') {
        ...
        if (!formData.Species_Name) formData.Species_Name = '';
        if (!formData['Unit']) formData['Unit'] = '';
        if (!formData.A_live) formData.A_live = '';
        ...
        supabaseClient.from(mapping.table).upsert(formData, { onConflict: mapping.idCol })
            .then(function (res) {
                ...
                if (res.error) {
                    ...
                    if (formType === 'Supported') {
                        var sFarmerId = formData.Farmer_ID;
                        var sYear = formData.Year;
...
function calcYearlyFromSupported(farmerId, yearCode) {
    ...
    var supports = (rawData.supported || []).filter(function (s) {
        return String(s.Farmer_ID) === String(farmerId)
            && String(s.Year || '') === String(year4);
    });
    ...
    var fertRecords = supports.filter(function (s) {
        var item = (s.Species_Name || '').toLowerCase();
...
function showSingleItemDetails(type, id) {
    ...
    var farmerName = '';
    var farmerIdVal = item.Farmer_ID;
    if (farmerIdVal) {
...
function exportDetailToExcel(farmerId) {
    ...
    // Sheet 4: Supported
    var relatedSupport = (rawData.supported || []).filter(function (s) { return String(s.Farmer_ID) === String(farmerId); });
...
function dbRenderFarmerDetail(farmer) {
    ...
    var relSupported = (rawData.supported || []).filter(function (s) { return (s.Farmer_ID) === farmer.Farmer_ID; });
...
function dbRenderChildTable(farmerId, type, idCol) {
    ...
    if (type === 'Plots') {
        ...
        dataSource = (rawData.supported || []).filter(function (r) { return (r.Farmer_ID) === farmerId; });
...
function dbPrintFarmerReport(farmerId) {
    ...
    var relSupported = (rawData.supported || []).filter(function (s) { return (s.Farmer_ID) === farmerId; });
...
function showKpiDrilldown(kpiType) {
    ...
    if (kpiType === 'survivalRate') {
        ...
        html += '<th>' + (isVi ? 'Số lượng' : 'Planted') + '</th>';
        html += '<th>' + (isVi ? 'Còn sống' : 'Alive') + '</th>';
        html += '<th>' + (isVi ? 'Tỷ lệ sống' : 'Rate') + '</th>';
        html += '<th style="min-width:100px"></th>';
        html += '</tr></thead><tbody>';
        spKeys.forEach(function(code, i) {
            var st = spStatsSR[code];
            var spName = getSpeciesName(code) || code;
            var rate = st.planted > 0 ? (st.alive / st.planted * 100) : 0;
            var rateStr = st.planted > 0 ? rate.toFixed(1) + '%' : (isVi ? 'Chưa ĐG' : 'N/A');
            var barClass = rate >= 80 ? '' : (rate >= 50 ? ' warning' : ' danger');
            var codeEsc = code.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
            html += '<tr class="kpi-drill-row" onclick="kpiDrillSurvivalSpecies(\'' + codeEsc + '\')">';
            html += '<td>' + (i + 1) + '</td>';
            html += '<td><strong>' + escapeHtml(spName) + '</strong></td>';
            html += '<td>' + (st.planted > 0 ? Math.round(st.planted).toLocaleString() : '-') + '</td>';
            html += '<td>' + (st.alive > 0 ? Math.round(st.alive).toLocaleString() : '-') + '</td>';
            html += '<td>' + rateStr + '</td>';
            html += '<td><div class="kpi-drill-bar"><div class="kpi-drill-bar-fill' + barClass + '" style="width:' + (st.planted > 0 ? rate : 0) + '%"></div></div></td>';
            html += '</tr>';
        });
        if (spKeys.length === 0) html += '<tr><td colspan="6" class="text-center text-muted py-3">' + (isVi ? 'Không có dữ liệu kiểm tra sống' : 'No survival check data') + '</td></tr>';
        if (spKeys.length > 0) {
            html += '<tfoot><tr><td colspan="2"><strong>' + (isVi ? 'Tổng' : 'Total') + '</strong></td>';
            html += '<td><strong>' + Math.round(totalPlantedSR).toLocaleString() + '</strong></td>';
            html += '<td><strong>' + Math.round(totalAliveSR).toLocaleString() + '</strong></td>';
            html += '<td><strong>' + overallSurv + '%</strong></td><td></td></tr></tfoot>';
        }
        html += '</table></div>';
...
function kpiDrillCompletionYearGroup(yearCode, groupCode) {
    ...
    activeList.forEach(function(f, i) {
        ...
        html += '<td>' + (i + 1) + '</td>';
        html += '<td>' + escapeHtml(f.Full_Name || f.Farmer_ID) + '</td>';
        html += '<td>' + (isDone ? '<i class="fas fa-check-circle text-success"></i> Done' : '<i class="fas fa-clock text-warning"></i> NY') + '</td>';
        html += '<td>' + (parseFloat(f.Total_Coffee_Area) || 0).toFixed(2) + '</td>';
        html += '</tr>';
    });
...
// Survival Rate Level 2: Species → Groups
function kpiDrillSurvivalSpecies(spCode) {
    var isVi = currentLang === 'vi';
    var spName = getSpeciesName(spCode) || spCode;
    // Build reverse name lookup for this species
    var spNamesSet = new Set([spCode]);
    var sp = speciesMap[spCode];
    if (sp) { if (sp.vi) spNamesSet.add(sp.vi.trim()); if (sp.en) spNamesSet.add(sp.en.trim()); }
    // Plot → Farmer map
    var plotFarmerMap2 = {};
    var plotGroupMap2 = {};
    (filteredData.plots || []).forEach(function(p) {
        var fid = String(p.Farmer_ID || '');
        var pid = String(p.Plot_Id || '');
        plotFarmerMap2[pid] = fid;
        var f = farmerMap[fid] || {};
        plotGroupMap2[pid] = f.Farmer_Group_Name || 'N/A';
    });
    // Latest Trees_Alive per (Plot_ID) for this spCode only
    var scLatest2 = {};
    (filteredData.survival_check || []).forEach(function(r) {
        var pk = (r.Plot_ID || '').trim(), sc = (r.Species_Code || '').trim();
        if (!pk || sc !== spCode) return;
        var rnd = String(r.Check_Round || '');
        if (!scLatest2[pk] || rnd >= scLatest2[pk].r)
            scLatest2[pk] = { r: rnd, alive: parseFloat(r.Trees_Alive) || 0 };
    });
    // Alive per group
    var grpAlive = {}, grpPlanted = {};
    Object.keys(scLatest2).forEach(function(pid) {
        var grp = plotGroupMap2[pid] || 'N/A';
        grpAlive[grp] = (grpAlive[grp] || 0) + scLatest2[pid].alive;
    });
    // Planted per group from support for this species
    var allGroups = new Set(Object.keys(grpAlive));
    (rawData.supported || []).forEach(function(s) {
        if (!spNamesSet.has((s.Species_Name || '').trim())) return;
        var f = farmerMap[s.Farmer_ID] || {};
        var grp = f.Farmer_Group_Name || 'N/A';
        if (!grpAlive[grp] && !grpPlanted[grp]) allGroups.add(grp);
        grpPlanted[grp] = (grpPlanted[grp] || 0) + (parseFloat(s.Quantity) || 0);
    });
    var grpKeys = Array.from(allGroups).filter(function(g) { return (grpAlive[g] || 0) > 0 || (grpPlanted[g] || 0) > 0; });
    grpKeys.sort(function(a, b) { return (grpPlanted[b] || 0) - (grpPlanted[a] || 0); });
    var totalAlive2 = grpKeys.reduce(function(s, g) { return s + (grpAlive[g] || 0); }, 0);
    var totalPlanted2 = grpKeys.reduce(function(s, g) { return s + (grpPlanted[g] || 0); }, 0);
    var overallRate2 = totalPlanted2 > 0 ? (totalAlive2 / totalPlanted2 * 100).toFixed(1) + '%' : 'N/A';
    var title2 = escapeHtml(spName) + ' — ' + (isVi ? 'Tỷ lệ sống theo nhóm' : 'Survival by Group');
    var html = '<p class="mb-2 text-muted" style="font-size:0.85rem;">' + (isVi ? 'Tổng: ' : 'Total: ') + '<strong>' + Math.round(totalAlive2).toLocaleString() + '/' + Math.round(totalPlanted2).toLocaleString() + ' (' + overallRate2 + ')</strong></p>';
    html += '<div class="table-responsive"><table class="table table-sm table-hover" style="font-size:0.82rem;">';
    html += '<thead class="table-custom-header"><tr><th>#</th>';
    ...
    html += '<th style="min-width:100px"></th>';
    html += '</tr></thead><tbody>';
    grpKeys.forEach(function(grp, i) {
        var alive = grpAlive[grp] || 0, planted = grpPlanted[grp] || 0;
        var rate = planted > 0 ? (alive / planted * 100) : 0;
        var rateStr = planted > 0 ? rate.toFixed(1) + '%' : (isVi ? 'Chưa ĐG' : 'N/A');
        var barClass = rate >= 80 ? '' : (rate >= 50 ? ' warning' : ' danger');
        var grpEsc = grp.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
        var spEsc = spCode.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
        var gLabel = adminMap[grp] ? (isVi ? adminMap[grp].vi : adminMap[grp].en) || grp : grp;
        html += '<tr class="kpi-drill-row" onclick="kpiDrillSurvivalSpeciesGroup(\'' + spEsc + '\',\'' + grpEsc + '\')">';
        html += '<td>' + (i + 1) + '</td>';
        html += '<td><strong>' + escapeHtml(gLabel) + '</strong></td>';
        html += '<td>' + (planted > 0 ? Math.round(planted).toLocaleString() : '-') + '</td>';
        html += '<td>' + (alive > 0 ? Math.round(alive).toLocaleString() : '-') + '</td>';
        html += '<td>' + rateStr + '</td>';
        html += '<td><div class="kpi-drill-bar"><div class="kpi-drill-bar-fill' + barClass + '" style="width:' + (planted > 0 ? rate : 0) + '%"></div></div></td>';
        html += '</tr>';
    });
    if (grpKeys.length === 0) html += '<tr><td colspan="6" class="text-center text-muted py-3">' + (isVi ? 'Không có dữ liệu' : 'No data') + '</td></tr>';
    if (grpKeys.length > 0) {
        html += '<tfoot><tr><td colspan="2"><strong>' + (isVi ? 'Tổng' : 'Total') + '</strong></td>';
        html += '<td><strong>' + Math.round(totalPlanted2).toLocaleString() + '</strong></td>';
        html += '<td><strong>' + Math.round(totalAlive2).toLocaleString() + '</strong></td>';
        html += '<td><strong>' + overallRate2 + '</strong></td><td></td></tr></tfoot>';
    }
    html += '</tbody></table></div>';
    kpiDrillPush(title2, html);
}

// Survival Rate Level 3: Species → Group → Farmers
function kpiDrillSurvivalSpeciesGroup(spCode, groupCode) {
    var isVi = currentLang === 'vi';
    var spName = getSpeciesName(spCode) || spCode;
    // Build reverse name lookup for this species
    var spNamesSet = new Set([spCode]);
    ...
    var farmersInGroup = (filteredData.farmers || []).filter(function(ff) { return (ff.Farmer_Group_Name || 'N/A') === groupCode; });
    var fidSet = new Set(farmersInGroup.map(function(ff) { return ff.Farmer_ID; }));
    // Plot → Farmer map (for this group only)
    var plotFarmerMap3 = {};
    (filteredData.plots || []).forEach(function(p) {
        var fid = String(p.Farmer_ID || '');
        if (fidSet.has(fid)) plotFarmerMap3[String(p.Plot_Id || '')] = fid;
    });
    // Latest Trees_Alive per plot for this spCode
    var scLatest3 = {};
    (filteredData.survival_check || []).forEach(function(r) {
        var pk = (r.Plot_ID || '').trim(), sc = (r.Species_Code || '').trim();
        if (!pk || sc !== spCode || !plotFarmerMap3[pk]) return;
        var rnd = String(r.Check_Round || '');
        if (!scLatest3[pk] || rnd >= scLatest3[pk].r)
            scLatest3[pk] = { r: rnd, alive: parseFloat(r.Trees_Alive) || 0, fid: plotFarmerMap3[pk] };
    });
    // Alive per farmer
    var farmerAlive = {};
    Object.keys(scLatest3).forEach(function(pk) {
        var rec = scLatest3[pk];
        farmerAlive[rec.fid] = (farmerAlive[rec.fid] || 0) + rec.alive;
    });
    // Planted per farmer from support for this species
    var farmerPlanted = {};
    (rawData.supported || []).forEach(function(s) {
        if (!fidSet.has(s.Farmer_ID)) return;
        if (!spNamesSet.has((s.Species_Name || '').trim())) return;
        farmerPlanted[s.Farmer_ID] = (farmerPlanted[s.Farmer_ID] || 0) + (parseFloat(s.Quantity) || 0);
    });
    // Build rows
    var rows = farmersInGroup.filter(function(ff) { return farmerAlive[ff.Farmer_ID] || farmerPlanted[ff.Farmer_ID]; });
    rows.sort(function(a, b) { return (farmerPlanted[b.Farmer_ID] || 0) - (farmerPlanted[a.Farmer_ID] || 0); });
    var totalAlive3 = rows.reduce(function(s, ff) { return s + (farmerAlive[ff.Farmer_ID] || 0); }, 0);
    var totalPlanted3 = rows.reduce(function(s, ff) { return s + (farmerPlanted[ff.Farmer_ID] || 0); }, 0);
    var overallRate3 = totalPlanted3 > 0 ? (totalAlive3 / totalPlanted3 * 100).toFixed(1) + '%' : 'N/A';
    var gLabel = adminMap[groupCode] ? (isVi ? adminMap[groupCode].vi : adminMap[groupCode].en) || groupCode : groupCode;
    var title3 = escapeHtml(spName) + ' / ' + escapeHtml(gLabel) + ' — ' + (isVi ? 'Tỷ lệ sống' : 'Survival');
    var html = '<p class="mb-2 text-muted" style="font-size:0.85rem;">' + (isVi ? 'Tổng: ' : 'Total: ') + '<strong>' + Math.round(totalAlive3).toLocaleString() + '/' + Math.round(totalPlanted3).toLocaleString() + ' (' + overallRate3 + ')</strong></p>';
    html += '<div class="table-responsive"><table class="table table-sm table-hover" style="font-size:0.82rem;">';
    html += '<thead class="table-custom-header"><tr><th>#</th>';
    html += '<th>' + (isVi ? 'Hộ dân' : 'Farmer') + '</th>';
    html += '<th>' + (isVi ? 'Số lượng' : 'Planted') + '</th>';
    ...
    html += '<th>' + (isVi ? 'Tỷ lệ sống' : 'Rate') + '</th>';
    html += '</tr></thead><tbody>';
    rows.forEach(function(ff, i) {
        var fid = ff.Farmer_ID;
        var alive = farmerAlive[fid] || 0, planted = farmerPlanted[fid] || 0;
        var rate = planted > 0 ? (alive / planted * 100).toFixed(1) + '%' : (isVi ? 'Chưa ĐG' : 'N/A');
        html += '<tr class="kpi-drill-row" onclick="showFarmerDetails(\'' + escapeHtml(fid) + '\')">';
        html += '<td>' + (i + 1) + '</td>';
        html += '<td>' + escapeHtml(ff.Full_Name || fid) + '</td>';
        html += '<td>' + (planted > 0 ? Math.round(planted).toLocaleString() : '-') + '</td>';
        html += '<td>' + (alive > 0 ? Math.round(alive).toLocaleString() : '-') + '</td>';
        html += '<td>' + rate + '</td>';
        html += '</tr>';
    });
    if (rows.length === 0) html += '<tr><td colspan="5" class="text-center text-muted py-3">' + (isVi ? 'Không có dữ liệu' : 'No data') + '</td></tr>';
    if (rows.length > 0) {
        html += '<tfoot><tr><td colspan="2"><strong>' + (isVi ? 'Tổng' : 'Total') + '</strong></td>';
        html += '<td><strong>' + Math.round(totalPlanted3).toLocaleString() + '</strong></td>';
        html += '<td><strong>' + Math.round(totalAlive3).toLocaleString() + '</strong></td>';
        html += '<td><strong>' + overallRate3 + '</strong></td></tr></tfoot>';
    }
    html += '</tbody></table></div>';
    kpiDrillPush(title3, html);
}
...
function showSurvivalFarmerDetail(farmerId, year) {
    ...
    treeRecs.forEach(function (s, i) {
        ...
        var species = s.Species_Name || '';
        var speciesName = (typeof getSpeciesName === 'function') ? getSpeciesName(species) : species;
        var qty = parseFloat(s.Quantity) || 0;
        var sy = s.Year || '';
        var supportBy = s.Program || '';
        var supportByLabel = resolveValue('Program', supportBy, 'Supported') || supportBy;
        var aVal = s.A_live;
        var hasEval = aVal !== null && aVal !== '' && aVal !== undefined && String(aVal).trim() !== '';
...
function saveSurvivalValue(inputEl) {
    ...
    supabaseClient.from('supported').update({ 'A_live': String(numVal) }).eq('Support_ID', supportId)
        .then(function (res) {
    ...
    if (totalResults === 0) {
        ...
        if (matchedSupported.length > 0) {
            ...
            matchedSupported.forEach(function (s) {
                ...
                html += '<td>' + escapeHtml(s.Year || '') + '</td>';
                html += '<td>' + escapeHtml(s.Species_Name || '') + '</td>';
...
function buildDataContext() {
    ...
    ctx += ser(support, ['Support_ID', 'Farmer_ID', 'Support_Type', 'Date', 'Species_Name', 'Quantity', 'Unit', 'Program', 'Year']);
